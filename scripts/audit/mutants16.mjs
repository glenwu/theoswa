...
runMutants([
  ...
  [F, '    sets.push(...discards(hand, count));', '    sets.push(...selections(hand, count));',
      '缺门整手垫牌时也挑最大的几张'],
  // ⚠️ 这一条现在【杀不掉，但不是没测到】：钉它的那两条测试（「赢不下的一墩里
  // 绝不拿鬼去凑张数」「缺门整手垫出去时也不许」）用的都是鬼，而「出鬼的代价」
  // 那条规则已经不再按手牌张数开关（见 mutants21），于是同一个局面被护了两遍，
  // 少一遍也不改结果。把护鬼那条改回去，这一条立刻又能杀 —— 覆盖是被盖住了，
  // 不是丢了。找不到「该垫的高牌不是鬼」而又明确是错的局面，就不硬凑一个。
