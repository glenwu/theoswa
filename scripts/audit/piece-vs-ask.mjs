...
// 这门外面还剩 ≤30 分就算刮不到分（口径同源码的 PIECE_COVER_MIN_POINTS）。
  ...
  hist.forEach((t, ti) => {
    ...
    for (const play of (t.plays ?? []).slice(1)) {
      ...
      for (const piece of pieces) {
        ...
        // 这门外面还剩多少分：满分 - 已出的（含本墩桌面）- 我手上的
        const suitMax = 50 - (rankCard === 5 ? 10 : (rankCard === 10 || rankCard === 13) ? 20 : 0);
        let seenPts = 0;
        for (let k = 0; k < ti; k++)
          for (const p2 of hist[k].plays ?? [])
            for (const x of p2.cards ?? []) if (ps(x) === suit) seenPts += cardPoints(x);
        for (const p2 of t.plays ?? []) {
          if (p2.seat === play.seat) continue;          // 我自己这一手还没出
          for (const x of p2.cards ?? []) if (ps(x) === suit) seenPts += cardPoints(x);
        }
        const minePts = hand.filter(x => ps(x) === suit).reduce((a, x) => a + cardPoints(x), 0);
        const atLarge = Math.max(0, suitMax - seenPts - minePts);
        const ok = heldHere >= 2 || pts >= 20 || lenHere <= 3 || atLarge <= 30;
        const won = t.winnerSeat === play.seat;
        if (ok) {
          add(`对手在求 · 合规（${label}）· ${heldHere >= 2 ? '我有两件' : pts >= 20 ? '≥20 分' : lenHere <= 3 ? '只剩两三支' : '这门刮不到分'}`);
        } else {
