...
// ============ 保底的鬼组合留到最后两轮 ============
//
// Glen 2026-08-30：「BOT 有大小鬼，对手只有小鬼，但没在最后两轮打，倒数三轮二轮
//   就把大小鬼打出来了，导致给对手撬底。」
//
// 实测 200 局（scripts/audit/joker-timing.mjs）：大小鬼都在手上的 140 次里，
// 55.7% 在倒数第三轮或更早就动了鬼；那些局的撬底率 20.5%，晚动鬼的只有 8.1%。
//
// ⚠️ 判据是【手上的鬼组合】，不是 holdsTopTrump —— 他说的那个牌型恰恰判不出来：
// assessBottomControl 把底牌里的牌一律当成还在对手手上（偏保守），
// 「大鬼 1 张在我手、1 张在外」这一档就卡在 threats < mineAtOrAbove 上过不去。
// ⚠️ 「这一墩的分到不了移庄线才罚」这个前提和上面 OVER_KILL 那条共用。
// 第一版漏了它，当场把他早先三条裁定顶红（60 分让掉就过线必须砍 / 到 90 爆底照杀 /
// 收下 20 分正好到移庄线该砍就砍）。那三条现在还在，别再动。
// ⚠️ fixture 必须让【只有鬼压得过】：首家领主 A，桌上再摆 20 分。
// 第一版让首家领 ♥4，我随手一张小主就压过去了 —— 那样两种写法都不出鬼，
// 测的根本不是这条规矩（变异测试当场戳穿：三条变异体全存活）。
function jokerTimingView(trumps, sides = [9, 7, 5]) {
  return followView({
    seat: 1, declarerSeat: 0,
    hand: [
      ...trumps.map((r, i) => T('H', r, i)),
      ...sides.map((r, i) => T('S', r, i + 20)),
    ],
    currentTrick: [
      { seat: 0, playSuit: 'TRUMP', cards: [T('H', 14, 80)] },   // 首家领主 A：只有鬼压得过
      { seat: 3, cards: [T('H', 13, 90)] },                      // ♥K = 10 分
      { seat: 2, cards: [T('H', 10, 91)] },                      // ♥10 = 10 分 → 桌上 20
    ],
  });
}

test('留鬼：大小鬼在手、离最后两轮还远 → 让掉这 20 分，鬼留着保底', () => {
  const play = chooseFollowCards(jokerTimingView([16, 15, 9, 7]))[0];
  assert.ok(play.rank !== 15 && play.rank !== 16,
    `保底比的是最后一墩，这时候兑现太早（实际打了 ♥${play.rank}）`);
});

// 反向对照①：手上只有一张鬼 —— 那不是保底组合，20 分该拿就拿。
// 这条也是 Glen 早先裁过的（「跟主牌墩：桌上 20 分时确实会用鬼拿下」），
// 这里再钉一次是为了把【组合】和【单张】分开。
test('留鬼：手上只有一张鬼 → 不是保底组合，20 分照拿', () => {
  const play = chooseFollowCards(jokerTimingView([15, 9, 7, 4]))[0];
  assert.equal(play.rank, 15, `单张鬼不构成保底组合（实际打了 ♥${play.rank}）`);
});

// 反向对照②：走到最后两轮了 —— 该兑现就兑现，再攥着就烂在手里。
// ⚠️ 这一条【钉不住边界本身】：手上只剩两张而又是大小鬼时，鬼是唯一候选，
// 罚不罚都得出，边界两侧行为一样。它钉的只是「别把鬼烂在手里」这个结果。
// 边界的【数值】由下面那条钉（三张牌时 >2 成立、>3 不成立）。
test('留鬼：走到最后两轮 → 该兑现就兑现', () => {
  const play = chooseFollowCards(jokerTimingView([16, 15], []))[0];
  assert.ok(play.rank === 15 || play.rank === 16,
    `最后两轮不打就烂在手里了（实际打了 ♥${play.rank}）`);
});

// ⚠️ JOKER_HOLD_LAST_TRICKS 这个【数值本身钉不住】，两头都试过：
//   · 手上只剩两张而又是大小鬼 → 鬼是唯一候选，罚不罚都得出，边界两侧一样；
//   · 手上三张（大小鬼 + 一张小主）→ 尾盘另有加分顶着，罚了也照样出鬼。
// 要有别的牌可打就至少三张，而三张那档又被尾盘加分盖住 —— 中间没有能分开的窗口。
// 所以只钉行为（早了不出 / 该出就出），不假装数值有覆盖。
...
//   10 支的情况，自己那时还有这门牌。」
//
// 插桩查过出口：真打出「A 砍 K」而这门还有别的牌的 51 次里，32 次（63%）
// 是从「打完只剩两三支」那条例外漏的。他当初给那条例外的理由是
//「打 A 后再捅多一支或两支就断了，【可以毙别人】，这个时候也可以吃」——
// 断门的价值全在「断了能毙」上。他这门要是比我的主还长，他一甩我拿什么毙？
//
// ⚠️ 判断「他能甩多长」用的是【最坏情况】（worstOpponentSuitLen），不是
// maxOpponentSuitEstimate。后者是按平均分布算的期望值，而真要甩的那一家恰恰是
// 攥着这门不放的那个 —— 实测那些场合期望值中位才 2.6、最大 5.6，
// 可实战真甩出来到 9 张，系统性地低估。
//
// ⚠️ 另一半「队友已经领先时照样把 K 的分送过去」由上面
// 【件不能乱出：队友已经稳赢这一墩】那条钉着，这里不重复造。
// ⚠️ 四家手牌数固定在 6：最坏情况取 min(这门外面还剩的, 他手上一共几张牌)，
// 手牌数一变门槛就跟着变，两臂就不是只差主牌长度了。
function aceOverKingView(trumpRanks, handCounts) {
  const view = followView({
    seat: 2, declarerSeat: 1,
    hand: [
      T('S', 14, 0), T('S', 9, 1), T('S', 7, 2),      // ♠A + 两张小的 → 打完只剩 2 支
      ...trumpRanks.map((r, i) => T('H', r, i + 10)),
      ...[8, 6].map((r, i) => T('D', r, i + 20)),
    ],
    currentTrick: [
      { seat: 1, playSuit: 'S', cards: [T('S', 13, 90)] },   // 对手领 ♠K
      { seat: 0, cards: [T('S', 3, 91)] },
    ],
    piecesView: {
      S: [{ rank: 14, status: 'mine' }, { rank: 14, status: 'unseen' },
          { rank: 13, status: 'seen' }, { rank: 13, status: 'unseen' }],
  ...
  for (const p of view.players) p.handCount = handCounts?.[p.seat] ?? 6;
  return view;
}

test('A 砍 K：我主牌只有三张，毙不动他一手长甩 → 别拿 A 去砍', () => {
  const play = chooseFollowCards(aceOverKingView([9, 8, 7]))[0];
  assert.notEqual(play.rank, 14,
    `断了门也毙不动他，「快断门可以吃」那条前提不成立（实际打了 ♠${play.rank}）`);
});

// 反向对照：同样打完只剩两支，但我主牌一大把 —— 他甩多长我都毙得住，
// 「断了能毙」这条前提成立，照 Glen 的原话可以吃。
// ⚠️ 少了这条，「快断门那条例外整个删掉」也能让上面那条绿。
test('A 砍 K：我主牌一大把，他甩多长都毙得住 → 照旧可以吃', () => {
  const play = chooseFollowCards(aceOverKingView([13, 12, 11, 10, 9, 8, 7]))[0];
  assert.equal(play.rank, 14,
    `断门之后我毙得动，那条例外本来就成立（实际打了 ♠${play.rank}）`);
});

// ⚠️ 这一条专门钉「用最坏情况、不用期望值」。同一手牌，两个量给的答案不一样：
//   期望值 maxOpponentSuitEstimate = 外面剩的 × 他手牌数 ÷ 暗牌总数 ≈ 4.4
//   最坏情况 worstOpponentSuitLen  = min(外面剩的, 他手牌数)          = 6
// 我有 5 张主：按期望值「毙得动」（4.4 ≤ 5）→ 会去砍；按最坏情况「毙不动」（6 > 5）
// → 不砍。实测那些场合期望值中位才 2.6、最大 5.6，而实战真甩出来到 9 张，
// 期望值系统性地低估 —— 风险判断不能用期望值。
test('A 砍 K：期望值说毙得动、最坏情况说毙不动 → 按最坏情况来，不砍', () => {
  const play = chooseFollowCards(aceOverKingView([10, 9, 8, 7, 6]))[0];
  assert.notEqual(play.rank, 14,
    `他可能攥着 6 张，我只有 5 张主，毙不干净（实际打了 ♠${play.rank}）`);
});

// ⚠️ 这一条钉「只算对手的手牌，不算队友的」：队友手上牌再多，他甩这门也不是威胁。
// 两个对手只剩 3 张牌，我有 4 张主 —— 毙得动，可以砍。
// 把队友算进来的话最坏情况会变成 12，就砍不了了。
test('A 砍 K：只算对手能攥多少，队友手牌多不算威胁', () => {
  const play = chooseFollowCards(
    aceOverKingView([10, 9, 8, 7], { 0: 12, 1: 3, 2: 4, 3: 3 })
  )[0];
  assert.equal(play.rank, 14,
    `两个对手各剩 3 张牌，我 4 张主毙得动（实际打了 ♠${play.rank}）`);
});
...
//
//   「闲家吃分为主更愿意亮件，我认为还是要遵守『件不能乱出』，只是庄家还要更严苛，
//     因为庄家需要保底，类似于『防守』。」
//
// 所以不是「闲家更松」，是【两边都守这道闸，庄家的线更高】：闲家 20 分松口，
// 庄家一方要 30 分。
// ⚠️ askedPieceView 默认 declarerSeat: 1，我是座 2 —— 座 2 和座 0 一队，
// 所以默认那一套 fixture 里我是【闲家】。下面把庄家挪到座 0 就换成庄家一方。
test('护件：桌上 20 分、我是闲家 → 到线了，可以砍', () => {
  const play = chooseFollowCards(askedPieceView({ table: [10, ['D', 5], ['D', 5]] }))[0];
  assert.equal(play.rank, 14, `闲家的线是 20 分（实际打了 ♠${play.rank}）`);
});

test('护件：同样桌上 20 分，但我在庄家一方 → 还不够，件留住', () => {
  const view = askedPieceView({ table: [10, ['D', 5], ['D', 5]] });
  view.declarerSeat = 0;          // 座 0 做庄 → 我（座 2）成了庄家一方
  const play = chooseFollowCards(view)[0];
  assert.notEqual(play.rank, 14,
    `庄家要保底，20 分还不值得把件交出去（实际打了 ♠${play.rank}）`);
});
...
// ⚠️ 这条【翻过来了】（2026-08-29）。原来断言的是「分都走了就可以亮 ♠A」，
// 那是把 Glen 的例子从「打 10 / 打 K」推广到了「中后段分被吃掉一部分，道理完全
// 一样」—— 推广是我们加的，不是他说的。他那天纠正了：
//   「甩牌之所以可怕，不仅是可以吃分，还在于它可以把本来很小的副牌升级成可以
//     大的牌，当然如果打 10 或 K，副牌可能能跑分的就不多，但一般的局，如果甩的
//     副牌足够长，还是可能吃到其它副牌的分。所以放件是需要非常谨慎的计算，
//     可放可不放的情况还是得选择不放。」
// 甩牌的价值不局限在这一门：甩长了，剩下的小牌升级成大牌，还能把别人手上
// 【别的门】的分逼出来。所以「这门的分被打掉了」根本不算安全。
// 判据因此从 suitPointsAtLarge（打掉的分）换成 sideSuitTotalPoints（天生的分）。
test('亮件：一般局里这门的分被拿走了 → 仍然不亮 ♠A（甩长了照样吃别门的分）', () => {
  const view = opponentProbeView([4, 5]);
  view.round.trickHistory = spadesPlayed([13, 13]);    // 同样两张，但是 20 分
  assert.notEqual(chooseFollowCards(view)[0].rank, 14,
    '打 2 的局，这门天生 50 分，分走了也不等于他甩出来没收益');
});
...
// ---- 起手牌第二档：「其次可以毙别人」（Glen 2026-08-30）----
//
//   「一般来说，如果有一手可以甩尾手，证明这门牌够强，经常会发生在庄家身上……
//     那极有可能已经可以把一门牌甚至两门都压断，那这个时候这门副牌场上还有，
//     就有机会用主去毙别人起手。」
//
// 三条同时成立：某门已经断了 + 那门场上还有牌 + 我是【长主】。
// 长主的线是他给的：「主牌共 36 张，平均 9 张，多过 9 张才算长主」。
//
// ⚠️ 观察点是【吊哪一张主】，不是【吊不吊主】：两种情况都会吊主（主够长本来就该吊），
// 差别在计划挂起时走 'tier' 档吊大牌（把对手的顶端逼出来），
// 没计划时走 'low' 档吊小牌。这一分岔在 drawingTrumpCard 的 mode 上。
function tailRuffEntryView(trumpRanks, extras = []) {
  return leadView({
    hand: [
      ...trumpRanks.map((r, i) => T('H', r, i)),
      ...[9, 7, 5].map((r, i) => T('S', r, i + 40)),   // 3 张黑桃，件全现 → 能甩
      // extras 为空时 ♦ ♣ 两门都断 —— 别人领那两门我就能毙下来起手
      ...extras,
    ],
    declarerSeat: 0, mySeat: 0,
    piecesView: {
...
test('甩尾手：10 张主（长主）+ 两门断 → 「可以毙别人」就是起手牌，计划成立', () => {
  const lead = chooseLeadCards(tailRuffEntryView([13, 12, 11, 10, 9, 8, 7, 6, 5, 4]))[0];
  assert.equal(lead.suit, 'H');
  assert.ok(lead.rank >= 11,
    `计划挂起时该吊大牌把对手顶端逼出来，不是吊小牌混（实际 ♥${lead.rank}）`);
});

// 反向对照：同样两门断，但主牌只有 9 张 —— 正好卡在他给的线上（「多过 9 张」才算）。
// 削不动对手就谈不上甩尾手，照常吊小牌就行。
test('甩尾手：9 张主不算长主 → 毙别人这一档不成立，照常吊小牌', () => {
  const lead = chooseLeadCards(tailRuffEntryView([13, 12, 11, 10, 9, 8, 7, 6, 5]))[0];
  assert.equal(lead.suit, 'H');
  assert.ok(lead.rank < 11,
    `9 张主不到长主线，没有甩尾手计划，不该为它吊大牌（实际 ♥${lead.rank}）`);
});

// 反向对照②：主够长，但【一门都没断】—— 没有断门就没有「毙别人」的机会，
// 这一档同样不成立。少了这条，把断门那个判据改成恒真也能过上面那条。
// ⚠️ 补的两张都是小牌，不能是 A：副 A 会顶上第三档（起手牌另有其人），
// 测的就不是断门这一条了。
test('甩尾手：主够长但一门都没断 → 没有毙别人的机会，这一档不成立', () => {
  const lead = chooseLeadCards(tailRuffEntryView(
    [13, 12, 11, 10, 9, 8, 7, 6, 5, 4],
    [T('D', 6, 70), T('C', 6, 71)],
  ))[0];
  // ⚠️ 这一臂的断言只能是「不去吊大牌」，不能像上面两条那样先断言领的是主牌：
  // 补了 ♦6 ♣6 之后手上有便宜的副牌可领，本来就不会去吊主。
  // 计划一旦成立（判据被改坏时就会），吊主提案 560 分立刻压过这些便宜牌，
  // 而且走 'tier' 档吊大牌 —— 所以这条断言照样分得开。
  assert.ok(!(lead.suit === 'H' && lead.rank >= 11),
    `一门都没断，谈不上用主毙别人起手，不该为它吊大牌（实际 ${lead.suit}${lead.rank}）`);
});
