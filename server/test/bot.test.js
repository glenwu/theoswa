...
//
// 牌面取自实测抓下来的真实现场（主 ♦ 打 2，桌面 T6 T7 T8 全是小主、一分没有，
// 我是末家）。原始那手里有双大鬼 —— 那是保底那家，按裁定【该盖就盖】；
// 把鬼拿掉就成了他说的「无所谓别人吊主」的手。两条一起才钉得住这条规矩。
function fourthHandTrumpView(bigs = []) {
  const hand = [
    ...bigs,
    card('fh-main2', 'D', 2), card('fh-side2', 'S', 2),   // 主级牌 + 副级牌
    card('fh-t7', 'D', 7), card('fh-t4', 'D', 4),         // 更便宜的主：跟得起，但赢不了
    card('fh-sa1', 'S', 14), card('fh-sa2', 'S', 14), card('fh-s11', 'S', 11),
    card('fh-s9', 'S', 9), card('fh-s7', 'S', 7),
    card('fh-h9', 'H', 9), card('fh-h9b', 'H', 9), card('fh-h8', 'H', 8),
  ];
  return {
    phase: 'PLAYING',
    declarerSeat: 0,
    players: [0, 1, 2, 3].map(s => ({ seat: s, team: s % 2, handCount: hand.length })),
    round: {
      roundNumber: 1, trumpSuit: 'D', rankCard: 2, kittyCount: 8,
      currentTrick: [
        { seat: 3, playSuit: 'TRUMP', cards: [card('fh-l', 'D', 6)] },
        { seat: 2, cards: [card('fh-m', 'D', 7)] },
        { seat: 0, cards: [card('fh-n', 'D', 8)] },
      ],
      trickHistory: [], lastTrick: null, turnSeat: 1,
      piecesView: {
        S: [14, 14, 13, 13].map(rank => ({ rank, status: 'unseen' })),
        H: [14, 14, 13, 13].map(rank => ({ rank, status: 'unseen' })),
...
test('末家吊主墩：一分没有、自己也不靠牌权 → 放一手，别花级牌', () => {
  const play = chooseFollowCards(fourthHandTrumpView());
  assert.ok(
    play.every(c => c.rank !== 2),
    `无分的吊主墩不值得用级牌买下来（实际打了 ${play[0].id}）`
  );
});

test('末家吊主墩：自己就是保底那手（双大鬼）→ 牌权有用，该盖照盖', () => {
  const play = chooseFollowCards(fourthHandTrumpView([
    card('fh-bj1', 'JOKER', 16), card('fh-bj2', 'JOKER', 16),
  ]));
  assert.equal(play[0].id, 'fh-side2',
    `顶端在我手上时这一墩的牌权是有用的，不该跟着放（实际打了 ${play[0].id}）`);
});
